---
title: Deploy MongoDB pada Docker dengan menggunakan Docker-Compose
author: "fadharpra"
date: 2023-05-31 13:06:00
cover:
  alt: Docker and MongoDB logo
  image: ../../assets/blog-cover/docker-mongodb.jpg
excerpt: Pada Artikel ini akan menjelaskan bagaimana Deploy database MongoDB pada Docker dengan menggunakan Docker-Compose.
tags:
  - Docker
  - Docker Compose
  - Containerization
...
<Aside type="note">
Docker Compose adalah alat yang digunakan untuk menjalankan dan mengelola aplikasi yang terdiri dari beberapa container Docker. Dengan menggunakan Docker Compose, Kalian dapat mengonfigurasi dan menjalankan beberapa container dalam satu lingkungan yang sama, mempermudah pengelolaan aplikasi secara keseluruhan.
</Aside>

<Aside type="caution">
Docker Compose versi terbaru tidak lagi memerlukan deklarasi `version`. Namun, jika Kalian menggunakan Docker Compose versi lama (sebelum 2.x), tetap sertakan `version: '3.9'` untuk kompatibilitas.
</Aside>
...
<Aside type="tip">
Pastikan Kalian telah menginstal Docker dan Docker Compose sebelum menjalankan perintah berikut.
</Aside>
...
Setelah membuat file `docker-compose.yml`, jalankan perintah berikut untuk memulai MongoDB:

```bash
docker-compose up -d
```

Verifikasi apakah container MongoDB sudah berjalan:
...
<Aside type="caution">
Jika container tidak berjalan, periksa logs dengan `docker logs mongodb-demo` untuk melihat penyebab issue yang ada.
</Aside>

Setelah MongoDB berjalan, Kalian dapat mengaksesnya dengan klien MongoDB atau tools seperti **Robo 3T**.

<Aside type="note">
Pastikan firewall dan aturan keamanan jaringan Kalian mengizinkan koneksi ke MongoDB jika ingin diakses dari luar container.
</Aside>